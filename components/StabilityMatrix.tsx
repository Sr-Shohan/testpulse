...
interface StabilityMatrixProps {
  tests: FlakyTestData[];
  builds: BuildInfo[];
  days: number;
  branch: string;
    ...
    <div className="glass-card p-6 min-h-[500px] h-full animate-fade-in flex flex-col relative overflow-hidden">
      ...
      <div className="flex flex-col xl:flex-row items-stretch xl:items-center justify-between gap-6 mb-8 bg-slate-900/40 -mx-6 -mt-6 p-6 border-b border-slate-800/50">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-100">Stability Matrix</h2>
                ...
                {recentBuilds.map((build: BuildInfo) => {
                  ...
                  return (
                    <td
                      ...
                      className={clsx(
                        "align-top p-1 text-center outline-none focus-visible:ring-2 focus-visible:ring-blue-500/60 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 rounded-lg",
                    ...
                    >
                      <div className={clsx(
                        ...
                        isPassed && "bg-emerald-500/20 border border-emerald-500/30 shadow-[inset_0_0_12px_rgba(16,185,129,0.1)]",
                        isFailed && "bg-rose-500/30 border border-rose-500/40 shadow-[0_0_15px_rgba(244,63,94,0.15)] animate-pulse-subtle",
                        status === 'NOT_RUN' && "border border-slate-700/20 bg-slate-800/40 opacity-50",
                        cellSelected &&
                        ...
                        <div className="w-full h-full flex items-center justify-center">
                          ...
                          {status === 'NOT_RUN' && <Minus className="w-3 h-3 text-slate-600" />}
                        </div>
                      </div>
                    </td>
                  );
      ...
      {selectedCell && typeof window !== "undefined" && createPortal(
        <>
          ...
          className="fixed z-[9999] w-80 rounded-xl border border-slate-700 bg-slate-900 p-4 shadow-[0_20px_50px_rgba(0,0,0,0.5)] duration-150 animate-in fade-in zoom-in-95 pointer-events-auto"
